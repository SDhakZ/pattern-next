...
export const selectActiveRing = (s) => {
  ...
  if (!cluster || !cluster.rings.length) return null;
  // Find ring matching activeRingId
  const activeRing = cluster.rings.find((r) => r.id === s.ui.activeRingId);
  // If activeRingId matches a ring in this cluster, use it; otherwise use first ring
  return activeRing ?? cluster.rings[0];
};
...
export const selectActiveReverseDecoration = (s) => {
  ...
  return (
    reverseDecorations.find((d) => d.id === activeReverseDecorationId) ?? null
  );
};
